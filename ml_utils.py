...
class MlUtils:
    ...
    #     bar_y = pd.np.linspace(color_min, color_max, n_colors)  # y coordinates for each of the n_colors bars
    #
    ...
    #     ax.set_yticks(pd.np.linspace(min(bar_y), max(bar_y), 3))  # Show vertical ticks for min, middle and max
    ...
    @classmethod
    def corrplot(cls, df, columns=None, size_scale=500, marker='s'):
        if columns:
            df = df[columns]
        data = df.corr()
